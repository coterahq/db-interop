...
export class DbtManifest {
  static fromConfig(object: any): Result<ManifestV11, ParseError | UnsupportedManifestVersionError> {
    ...
    switch(parsed.data.metadata.dbt_schema_version) {
      ...
      default:
        return err(new UnsupportedManifestVersionError(parsed.data.metadata.dbt_schema_version))
    }
  }
...
export class ManifestV11 {
  ...
  models(): V11ModelNode[] {
    return Object.values(this.nodes).filter(node => node.resource_type === "model") as V11ModelNode[]
  }
}